...
class PCA():
    ...
    def get_components(self, data):
        ...
        eigen_values1, eigen_vectors1 = np.linalg.eigh(covariance_matrix)

        eigen_values, eigen_vectors = self.get_eigen_values_and_vectors(covariance_matrix)

        sorted_indices = np.argsort(eigen_values1)[::-1].astype(int)

        # sorted_eigen_values = eigen_values[sorted_indices]

        # print(type(sorted_indices))
        #
        sorted_eigen_vectors = eigen_vectors1[:, sorted_indices]
        #
        final_eigen_vectors1 = np.array(sorted_eigen_vectors[:, 0:self.num_components]).T

        final_eigen_vectors = np.array(eigen_vectors)

        reduced_data = np.dot(final_eigen_vectors, centered_data.T).T
